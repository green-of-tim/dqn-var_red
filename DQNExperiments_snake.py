...
for runId in np.arange(nRuns):
    ...
    logs=agent1.train(n_epochs=nEpochs,verbose=5)
    # timesDQN[runId]=time.time()-time0
    logsDQN[runId,:]=logs['meanRewards']
    with open('D:\Experiments VRML\DQNNNNNN_VR'+envName+'TestStab_' + 'DQN_VR' +'_nEpochs'+str(nEpochs)+'_lr'+str(lr)+'_'+nameAppendix+'_'+str(runId)+'.pkl','wb') as f:
        pickle.dump({'meanRewards':logs['meanRewards'], 'variances':logs['variances'], 'qv':logs['Qvalues'], 'norms':logs['norms']},f)
    
    logs=agent2.train(n_epochs=nEpochs,verbose=5)
    # timesDQN[runId]=time.time()-time0
    logsDQN[10+runId,:]=logs['meanRewards']
    with open('D:\Experiments VRML\DQNNNNNN'+envName+'TestStab_' + 'DQN_VR' +'_nEpochs'+str(nEpochs)+'_lr'+str(lr)+'_'+nameAppendix+'_'+str(runId)+'.pkl','wb') as f:
...
ax.plot(np.arange(nEpochs),logsDQN[0:10,:])
ax.plot(np.arange(nEpochs),logsDQN[10:20,:])
ax.legend(['DQN VR', 'DQN'])

plt.savefig('plot Snake rewards 10mean.pdf',bbox_inches='tight')
